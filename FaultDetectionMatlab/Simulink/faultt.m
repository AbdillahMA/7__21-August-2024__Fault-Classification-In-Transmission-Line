...
for GroundResistance = GroundResistance_values
    ...
    for i = 1:length(faultTypes)
        ...
        model = 'RBFNNfaultdet'; % replace with your model name
        open_system(model);
        ...
        % Save the maximum values
        results.(faultName).(['GroundResistance_', GroundResistanceField]).PhaseA = max(coefA);
        results.(faultName).(['GroundResistance_', GroundResistanceField]).PhaseB = max(coefB);
        results.(faultName).(['GroundResistance_', GroundResistanceField]).PhaseC = max(coefC);
        results.(faultName).(['GroundResistance_', GroundResistanceField]).Ground = max(coefG);
...
writetable(resultTable, 'fault_simulation_results_with_GroundResistance_after_transmission.csv');

% Save the results to a .mat file
save('fault_simulation_results_with_GroundResistance_after_transmission.mat', 'results');
